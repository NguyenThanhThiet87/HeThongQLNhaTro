...
public class HoaDonThanhToanController : ControllerBase
{
    ...
    public HoaDonThanhToanController(AppDbContext context, IHubContext<ChatHub> hubContext)
    {
        _context = context;
    ...
    public async Task<IActionResult> ThanhToanHoaDon([FromBody] LichSuThanhToan model)
    {
        ...
        // Kiểm tra xem đã trả đủ chưa để update trạng thái hóa đơn
    ...
    public async Task<IActionResult> GetSoDienNuocCu([FromQuery] int maPhong, int monthCurrent, int yearCurrent)
    {
        try
        {
            var temp = _context.ChiSoDienNuocs.FirstOrDefault(csdn => csdn.MaPhong == maPhong && csdn.Thang == monthCurrent && csdn.Nam == yearCurrent);
            if (temp == null)
            {
                int monthOld = monthCurrent;
                int yearOld = yearCurrent;
                if (monthCurrent == 1)
                {
                    monthOld = 12;
                    yearCurrent -= 1;
                ...
                ChiSoDienNuoc csdnOld = _context.ChiSoDienNuocs.FirstOrDefault(csdn => csdn.MaPhong == maPhong && csdn.Thang == monthOld && csdn.Nam == yearOld);
                ...
                return Ok(new ApiResponse<Object>(
                    ...
                    new
                    {
                        DienCu = csdnOld.CsdienMoi,
                        NuocCu = csdnOld.CsnuocCu
                    }
                    ));
            }
            else
    ...
    public async Task<IActionResult> SaveSoDienNuoc([FromBody] ChiSoDienNuocDto model)
    {
        ...
        try
        {
            var temp = _context.ChiSoDienNuocs.FirstOrDefault(csdn => csdn.MaPhong == model.MaPhong && csdn.Thang == model.Thang && csdn.Nam == model.Nam);
            if (temp == null)
            {
                int? monthOld = model.Thang;
                int? yearOld = model.Nam;
                if (model.Thang == 1)
                {
                    monthOld = 12;
                    model.Nam -= 1;
                ...
                ChiSoDienNuoc csdnOld = _context.ChiSoDienNuocs.FirstOrDefault(csdn => csdn.MaPhong == model.MaPhong && csdn.Thang == monthOld && csdn.Nam == yearOld);

                ChiSoDienNuocDto csdnNew = new ChiSoDienNuocDto
                {
                    MaPhong = model.MaPhong,
                    Thang = model.Thang,
                    Nam = model.Nam,
                    CsdienCu = csdnOld != null ? csdnOld.CsdienMoi : 0,
                    CsnuocCu = csdnOld != null ? csdnOld.CsnuocMoi : 0,
                    CsdienMoi = model.CsdienMoi,
                    CsnuocMoi = model.CsnuocMoi
                };

                var csdnEntity = new ChiSoDienNuoc
                {
                    MaPhong = csdnNew.MaPhong,
                    Thang = csdnNew.Thang,
                    Nam = csdnNew.Nam,
                    CsdienCu = csdnNew.CsdienCu,
                    CsnuocCu = csdnNew.CsnuocCu,
                    CsdienMoi = csdnNew.CsdienMoi,
                    CsnuocMoi = csdnNew.CsnuocMoi,
                };
                _context.ChiSoDienNuocs.Add(csdnEntity);
                await _context.SaveChangesAsync();

                List<HopDongThue> hdtList = _context.HopDongThues.Where(hdt => hdt.MaPhong == csdnNew.MaPhong && hdt.MaPhongNavigation.MaTtphong == TrangThaiPhongConstant.DangThue && hdt.MaTthopDong == TrangThaiHopDongConstant.DangHieuLuc)
                    .Include(hd => hd.HopDongNguoiThues)
                ...
                (
                    false,
                    "Hợp đồng xảy ra xung đột",
                    null
                ));
                }
                else if (hdtList.Count == 0)
                {
                    return Ok(new ApiResponse<Object>
                (
                    false,
                ...
                HopDongThue hdt = hdtList[0];

                var tienDien = (csdnNew.CsdienMoi - csdnNew.CsdienCu) * hdt.GiaDien;
                var tienNuoc = (csdnNew.CsnuocMoi - csdnNew.CsnuocCu) * hdt.GiaNuoc;

                HoaDon hd = new HoaDon
                {
                    MaHopDong = hdt.MaHopDong,
                    NgayLap = DateTime.Now,
                    TienDien = (csdnNew.CsdienMoi - csdnNew.CsdienCu) * hdt.GiaDien,
                    TienNuoc = (csdnNew.CsnuocMoi - csdnNew.CsnuocCu) * hdt.GiaNuoc,
                    MaTthoaDon = TrangThaiHoaDonConstant.ChuaThanhToan,
                    TienPhong = hdt.GiaThue,
                    TongTien = hdt.GiaThue + (tienDien + tienNuoc),
                    MaChiSo = csdnEntity.MaChiSo
                };
                ...
                {
                    MaNd = hdt.HopDongNguoiThues.First().MaNt,
                    TieuDe = "Hóa đơn mới 📑",
                    NoiDung = $"Bạn có hóa đơn mới cho phòng {hdt.MaPhongNavigation?.SoPhong}. Tổng cộng: {hd.TongTien:N0}đ. Vui lòng kiểm tra và thanh toán.",
                    DaDoc = false,
                    NgayTao = DateTime.Now,
                    LoaiTb = "TienPhong",
                    MaThucThe = hd.MaHoaDon
                };
                ...
                await _hubContext.Clients.User(hdt.HopDongNguoiThues.First().MaNt.ToString())
    ...
    public async Task<IActionResult> GetDanhSachThangHoaDon([FromQuery] int maDayNt)
    {
        try
        {
            // Lấy tháng, năm nhỏ nhất của dãy nhà trọ
    ...
    public async Task<IActionResult> GetDoanhThu([FromQuery] int maNd)
    {
        try
        {
            ...
            // Lấy tất cả hóa đơn của các dãy trong tháng hiện tại
            ...
            // Lấy tất cả hóa đơn của các dãy trong tháng trước
    ...
    public async Task<IActionResult> GetThongKeChiTieu([FromQuery] int maNd)
    {
        try
        {
            // Lấy 6 hóa đơn gần nhất của người dùng
