...
public class PhongNhaTroController : ControllerBase
{
    ...
    public PhongNhaTroController(AppDbContext context, IPhotoService photoService, IConfiguration config)
    {
        _context = context;
    ...
    public async Task<IActionResult> GetPhong([FromQuery] int? maPhong)
    {
        var result = await _context.Phongs
            ...
            .Select(p => new
            {
                ...
                HopDongThue = p.HopDongThues.Where(hd => hd.MaTthopDong == TrangThaiHopDongConstant.DangHieuLuc).Select(hd => new
                {
                    MaHopDong = hd.MaHopDong,
                    MaTthopDong = hd.MaTthopDong,
                    TrangThai = hd.MaTthopDongNavigation.TenTthopDong,
                    NgayBdhl = hd.NgayBdhl,
                    NgayKthl = hd.NgayKthl,
                    DsNguoiThue = hd.HopDongNguoiThues.Select(hdnt => new
                    {
                        MaNt = hdnt.MaNtNavigation.MaNt,
                        HoTen = hdnt.MaNtNavigation.MaNtNavigation.HoTen,
                        SoDt = hdnt.MaNtNavigation.MaNtNavigation.SoDt,
                        Avatar = hdnt.MaNtNavigation.MaNtNavigation.Avatar
                    }).ToList()
                }).FirstOrDefault()
            })
            .FirstOrDefaultAsync();
