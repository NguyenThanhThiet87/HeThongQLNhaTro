...
import { addThanhVienHopDongApi, addThanhVienHopDongExistedApi } from "../../../api/HopDong";
...
export default function OTPVerificationScreen_ThemThanhVien({ route }) {
    ...
    const { data, isExistAccount } = route.params;
    ...
    const handleVerify = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                {
                    const apiResult = await addThanhVienHopDongExistedApi(data);
                    if (apiResult.success) {
                        toast.success("Thành viên đã được thêm vào hợp đồng thành công!");
                        navigation.navigate("ChiTietHopDong", { maHopDong: data.maHopDong });
                    } else {
