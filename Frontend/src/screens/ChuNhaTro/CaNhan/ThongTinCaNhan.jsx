...
export default function ThongTinCaNhan() {
  ...
  const { user, loading } = useUserCntProfile();
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: COLORS.bgLight }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={[styles.profileSection, { backgroundColor: COLORS.bgLight }]}>
          ...
          <TouchableOpacity style={styles.editProfileBtn} onPress={() => navigation.navigate("ThayDoiThongTinCaNhan", { maNd: user?.maChuNt })}>
        ...
        <View style={styles.detailsSection}>
          ...
          <InputGroup
            ...
            value={user?.soGpkd || "Chưa cập nhật"}
            iconName="description"
            enable={false}
          />
          <InputGroup
            label="SỐ TÀI KHOẢN NGÂN HÀNG"
            value={user?.soTk ? `${user?.tenNh}: ${user?.soTk}` : "Chưa cập nhật"}
            iconName="person"
            numberOfLines={5}
            enable={false}
          />
