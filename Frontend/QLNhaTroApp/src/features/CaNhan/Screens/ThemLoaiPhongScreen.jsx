...
import { taoLoaiPhongApi } from "../../../api/PhongTro";
...
export default function ThemLoaiPhongScreen() {
    ...
    const handleSave = async () => {
        ...
        if (isValid) {
            ...
            const result = await taoLoaiPhongApi(data);
            if (result.success) {
                navigation.goBack();
                toast.success("Loại phòng đã được tạo thành công!");
            } else {
                toast.error("Lỗi tạo loại phòng: " + result.message);
            }
        }
        setLoading(false);
    }
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: "#f6f8f8" }]}>
            ...
            <AppHeader
                ...
                center={
                    <Text style={[styles.headerTitle, { color: theme.text }]}>Thêm Loại phòng</Text>
                }
                right={
            ...
            <View style={[styles.footer, { borderTopColor: COLORS.primaryAlpha10 }]}>
                <View style={styles.footerButtons}>
                    <TouchableOpacity style={[styles.btnSecondary, { borderColor: COLORS.primaryAlpha20 }]}>
