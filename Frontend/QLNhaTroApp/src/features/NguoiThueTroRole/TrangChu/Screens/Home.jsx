...
import { getDeadlineDate } from '../../../../utils/formatNgaySinh';
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: COLORS.bgLight }]}>
      ...
      <View style={[styles.header, { backgroundColor: COLORS.bgLight }]}>
        <View style={styles.userInfo}>
          <Image
            source={{ uri: tenantDetails?.avatar }}
      ...
      <KeyboardAvoidingView
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
            ...
            <View style={styles.expenseFooter}>
              ...
              </View>
              {
                latestBill == null ? null : (
                  <TouchableOpacity style={styles.payBtn} onPress={() => navigation.navigate('ThanhToanHoaDonScreen', { maHd: latestBill.maHoaDon })}>
          ...
          <ActivityItem icon="build" title="Sửa bóng đèn phòng tắm" date="20 Th09, 2023" status="Hoàn thành" statusColor="#16a34a" />
          <ActivityItem icon="local-drink" title="Đặt 02 bình nước 20L" date="Vừa xong" status="Đang xử lý" statusColor="#2563eb" />
...
const ActivityItem = ({ icon, title, date, status, statusColor }) => {
  const { COLORS } = useTheme();
  const styles = createStyles(COLORS);
